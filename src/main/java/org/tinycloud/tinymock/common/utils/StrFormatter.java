...
public class StrFormatter {
    ...
    public static String formatWith(String strPattern, String placeHolder, Object... argArray) {
        if (StrUtils.isBlank(strPattern) || StrUtils.isBlank(placeHolder) || ObjectUtils.isEmpty(argArray)) {
            return strPattern;
        }
        ...
        for (int argIndex = 0; argIndex < argArray.length; argIndex++) {
            ...
            if (delimIndex > 0 && strPattern.charAt(delimIndex - 1) == C_BACKSLASH) {// 转义符
                if (delimIndex > 1 && strPattern.charAt(delimIndex - 2) == C_BACKSLASH) {// 双转义符
                    ...
                    sbuf.append(ObjectUtils.getDisplayString(argArray[argIndex]));
    ...
    public static String format(CharSequence template, Map<?, ?> map, boolean ignoreNull) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            value = ObjectUtils.getDisplayString(entry.getValue());
